...
def main(args):
    ...
    #### 处理level1
    level1_df = merge_df.loc[:, ['query', 'Functional_Category'] + samples]
    ...
    level1_split.drop_duplicates(subset=['query', 'Functional_Category'], inplace=True)
    ...
    for row in level1_split.loc[:, ['query', 'Functional_Category']].values:
        gene, _class = row
    ...
    for row in level2_df.loc[:, ['query', 'OG_Description']].values:
        gene, _class = row
    ...
    for row in og_df.loc[:, ['query', 'Ortholog_Group']].values:
        gene, _class = row
