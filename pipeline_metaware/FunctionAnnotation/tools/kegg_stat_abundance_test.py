...
def main(args):
    ...
    gene_df = pd.read_csv(args['abundance_table'], sep='\t').rename(columns={'Reference_ID': 'Query_id'})
    merge_df = pd.merge(anno_df, gene_df, on='Query_id', how='outer').fillna('Others')

    ################# 处理ko
    ko_df = merge_df.loc[:, ['Query_id', 'KO_ID'] + samples]
    ko_df.drop_duplicates(subset=['Query_id', 'KO_ID'], inplace=True)
    stat_ko = ko_df.groupby('KO_ID').sum()
    ...
    ################# 处理ec
    ec_df = merge_df.loc[:, ['Query_id', 'KO_EC'] + samples]
    ec_split = ec_df.drop('KO_EC', axis=1).join(ec_df['KO_EC'].str.split(',', expand=True).stack().reset_index(level=1, drop=True).rename('KO_EC'))
    ec_split.drop_duplicates(subset=['Query_id', 'KO_EC'], inplace=True)  # gene-muti_ko-same_ec
    ...
    ################### 处理mo
    mo_df = merge_df.loc[:, ['Query_id', 'Module'] + samples]
    ...
    mo_split.drop_duplicates(subset=['Query_id', 'Module'], inplace=True)  # gene-muti_ko-same_ec
    ...
    #################### 处理pathway
    pathway_df = merge_df.loc[:, ['Query_id', 'KO_PATHWAY'] + samples]
    ...
    ### 层级1
    level1_df = pathway_list.loc[:, ['Query_id', 'KO_Pathway_Level1'] + samples]
    level1_df.drop_duplicates(subset=['Query_id', 'KO_Pathway_Level1'], inplace=True)
    ...
    ## 层级2
    level2_df = pathway_list.loc[:, ['Query_id', 'KO_Pathway_Level2'] + samples]
    level2_df.drop_duplicates(subset=['Query_id', 'KO_Pathway_Level2'], inplace=True)
    ...
    ## 层级3
    level3_df = pathway_list.loc[:, ['Query_id', 'map'] + samples].rename(columns={'map': 'KO_Pathway_Level3'})
    level3_df.drop_duplicates(subset=['Query_id', 'KO_Pathway_Level3'], inplace=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kegg_anno', help='基因-kegg功能注释结果')
    parser.add_argument('--abundance_table', help='基因-丰度信息')
