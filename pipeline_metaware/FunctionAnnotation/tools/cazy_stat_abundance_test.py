...
def main(args):
    ...
    abundance_df = pd.read_csv(args['abundance_table'], sep='\t').rename(columns={'Reference_ID': 'Query_id'})
    merge_df = pd.merge(anno_df, abundance_df, on='Query_id', how='outer').fillna('Others')


    ########################### 处理6大通路文件
    cazy_df = merge_df.loc[:, ['Query_id', 'CAZy_Family'] + samples]
    # cazy_df.rename(columns={'family': 'CAZy_Family'})
    ...
    ## level1水平
    cazy_level1_df = cazy_split.loc[:, ['Query_id', 'CAZy_Modules'] + samples]
    cazy_level1_df.drop_duplicates(subset=['Query_id', 'CAZy_Modules'], inplace=True)
    ...
    stat_cazy_level1_re.reset_index().to_csv('{result_suffix}.relative.level1.xls'.format(**args), sep='\t')
    stat_cazy_level1_ab.reset_index().to_csv('{result_suffix}.absolute.level1.xls'.format(**args), sep='\t')
    # 分组结果
    ...
    ## level2 水平
    cazy_level2_df = cazy_split.loc[:, ['Query_id', 'CAZy_Family'] + samples]
    cazy_level2_df['CAZy_Family'] = cazy_level2_df['CAZy_Family'].apply(lambda x: re.search(r'[a-zA-Z0-9]+', x).group())  # 将亚家族转换为家族
    cazy_level2_df.drop_duplicates(subset=['Query_id', 'CAZy_Family'], inplace=True)
    ...
    stat_cazy_level2_re.reset_index().to_csv('{result_suffix}.relative.level2.xls'.format(**args), sep='\t')
    stat_cazy_level2_ab.reset_index().to_csv('{result_suffix}.absolute.level2.xls'.format(**args), sep='\t')
    # 分组结果
    ...
    stat_cazy_level2_group.reset_index().to_csv('{result_suffix}.relative.group.level2.xls'.format(**args), sep='\t')


    # # ################### 处理酶相关
    # ec_df = merge_df.loc[:, ['Query_id', 'ec'] + samples]
    ...
    # ec_stat = cazy_level2_df.groupby('ec').sum()
    
    # ec_stat_re = ec_stat / ec_stat.sum(axis=0)
    # ec_stat_ab = (ec_stat / ec_stat.sum(axis=0)) * max(ec_stat.sum(axis=0))
    # ec_stat_re.reset_index().to_csv('{result_suffix}.relative.ec.xls'.format(**args), sep='\t')
    # ec_stat_ab.reset_index().to_csv('{result_suffix}.absolute.ec.xls'.format(**args), sep='\t')
    # # 分组结果
    # ec_stat_group = get_target_group_df(ec_stat_re, group_info)
    # ec_stat_group = ec_stat_group / ec_stat_group.sum(axis=0)
    # ec_stat_group.reset_index().to_csv('{result_suffix}.relative.group.ec.xls')
